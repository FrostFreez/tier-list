...
function Tier() {
  ...
  function loadTiers() {
    ...
    result.tiers.map((tier) => {
      tiers.push(new Tiers(tier.name, tier.color, tier.members));
    });
  }
  ...
  return (    
    <div onScroll={grow} onLoad={tierer}>
      ...
      <div id="description" class="description">
        ...
        <p id="tierlistDescription">HI amigos</p>
      </div>
      <div style={{display: 'flex'}}>
        <div id={"tierlist"} className="tierlist"></div>
        <div id="rest" className="rest" onDrop={drop} onDragOver = {allowDrop}></div>
      </div>
      <div id="creation" className="inactiveCreation">
        <input id="memberName" className="panelTop" type="text" placeholder="placeholder 1"/>
        <input id="memberURL" className="panelTop" type="text" placeholder="placeholder 2" onChange={changeImage}/>
        <div id="imgContainer">
